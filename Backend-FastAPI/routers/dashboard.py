...
def get_hod_summary(
    ...
    pending_count  = db.query(func.count(Intervention.id)).filter(
        Intervention.status == 'pending').scalar()
    resolved_count = db.query(func.count(Intervention.id)).filter(
        Intervention.status == 'resolved').scalar()

    print(f"HOD: total={total_students}, high={high_count}, medium={medium_count}, low={low_count}")
